...
def process_map_data(old_data):
    ...
    for value in old_data:
        ...
        collision = (value & OLD_COLLISION_MASK) >> 10 # leaving be
        elevation = (value & OLD_ELEVATION_MASK) >> 12 # leaving be

        # commenting out, not changing elevations
        # if elevation == 15:
        #     elevation = 7
        if metatile_id >= 512:
            metatile_id = metatile_id + (512 * 1) # changed from *3 to *1 as only increasing from 1024 to 2048, not to 4096
